...
def exec(individualsnf=sheetoperator.individualsnf, individualsf=sheetoperator.individualsf, individualspet=sheetoperator.individualspet, individualsgr=sheetoperator.individualsgr, equipsnf=sheetoperator.equipsnf, equipsf=sheetoperator.equipsf):
	...
	while i <=2:
		...
		rcontents.insert(17, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][3])))
		rcontents.insert(26, f'"t{i+1}"')
		rcontents.insert(37, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][3])))
		rcontents.insert(50, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][4])))
		write(rcontents)
		i += 1
	# PODOIUM CONTROL
	...
	while i <= sheetoperator.individualsf[0]-1:
		...
		rcontents.insert(9, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][1])))
		rcontents.insert(12, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][2])))
		rcontents.insert(18, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][3])))
		rcontents.insert(27, f'"t{i+1}"')
		rcontents.insert(38, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][3])))
		rcontents.insert(51, str(nonetodash.nonetodash(sheetoperator.tot[indfindex][4])))
		write(rcontents)
		i += 1
	# LIST CONTROL
